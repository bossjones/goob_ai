...
from src.goob_ai.utils.file_functions import (
    aio_read_jsonfile,
    aio_json_loads,
    run_aio_json_loads,
    sort_dir_by_mtime,
...
def test_get_all_media_files_to_upload(mocker):
    mock_tree = mocker.patch("src.goob_ai.utils.file_functions.tree", return_value=["file1", "file2"])
    mock_filter_media = mocker.patch("src.goob_ai.utils.file_functions.filter_media", return_value=["file1"])
...
def test_filter_media(mocker):
    mock_filter_images = mocker.patch("src.goob_ai.utils.file_functions.filter_images", return_value=["file1.png"])
    mock_filter_videos = mocker.patch("src.goob_ai.utils.file_functions.filter_videos", return_value=["file2.mp4"])
...
def test_filter_pdf(mocker):
    mock_filter_pdfs = mocker.patch("src.goob_ai.utils.file_functions.filter_pdfs", return_value=["file1.pdf"])
...
def test_rich_display_meme_pull_list(mocker):
    mock_console = mocker.patch("src.goob_ai.utils.file_functions.Console.print")
    df = pd.DataFrame({
        "Account": ["acc1"],
        "Social": ["soc1"],
...
def test_fix_path(mocker):
    mock_is_file = mocker.patch("src.goob_ai.utils.file_functions.is_file", return_value=True)
    mock_tilda = mocker.patch("src.goob_ai.utils.file_functions.tilda", return_value="/home/user/file")
    result = fix_path("~")
