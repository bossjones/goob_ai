...
#     # ctx.run("python ./goob_ai/api/backend_pre_start.py")
#     # ctx.run("python ./goob_ai/initial_data.py")
#     # ctx.run("python goob_ai/dev_serve.py")
...
pip install -r requirements-doc.txt
    ...
    click.secho(
        "[install] install editable version of cerebro-bot",
        fg=COLOR_SUCCESS,
    )
    ctx.run("pip install -e .")
...
def rsync(ctx, loc="local", verbose=0, cleanup=False):
    ...
    _cmd = r"""
cd && rsync -r --exclude cerebro-bot_venv --exclude .vagrant --exclude .git --exclude .env /srv/vagrant_repos/goob_ai/ ~/goob_ai/ && sudo chown vagrant:vagrant -R ~vagrant && cd ~/cerebro-bot && ls -lta
    """
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python goob_ai/web.py' | awk '{print $2}' | head -1
    """
