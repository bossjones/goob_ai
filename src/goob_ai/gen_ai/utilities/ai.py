...
#     def __init__(self, bot: AsyncGoobBot, channel_id: str, temperature: float = 0, return_messages: bool = True):
#         """
...
#     def __init__(self, bot: AsyncGoobBot, namespace: str):
#         """
...
#         """
#         LOGGER.debug(bot, "Loading LLM Query")
...
#         self.qap = PromptTemplate(template=QA_V2, input_variables=["context", "question"])
...
#         self.doc_chain = load_qa_chain(self.streaming_llm, chain_type="stuff", prompt=self.qap)

#         pinecone.init(api_key=bot.pinecone_api_key, environment=bot.pinecone_env)
#         self.embeddings = OpenAIEmbeddings(model="text-embedding-ada-002", openai_api_key=bot.openai_api_key)
